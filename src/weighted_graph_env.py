...
visited_vertices: <list> list of vertices visited.
...
get_local_unvisited_vertices_gt
----------------------------
...
get_all_unvisited_vertices_with_path_cost
--------------------------------------
...
class WeightedGraphEnv:
    ...
    def __init__(self,
        ...
        # -- read the graph
        self._initialize_ground_truth_graph(edgeFileName)
    ...
    # --------------------------------------------------------------------------
    # Function: get_local_unvisited_vertices_gt
    # --------------------------------------------------------------------------
    # Returns the set of unvisited vertices from the local neighborhood
    # --------------------------------------------------------------------------
    # Input:
    #   vertex: A node of the graph environment
    #   debug_print: Prints on screen if true
    # --------------------------------------------------------------------------
    def get_local_unvisited_vertices_gt(self, vertex, debug_print=False):
        local_vertices = self.get_local_nodes_gt(vertex)

        del_idx = []
        for local_nodes_idx, vertex in enumerate(local_vertices):
            if vertex[0] in self.visited_vertices:
                del_idx.append(local_nodes_idx)

        if debug_print:
            print("ENV GRAPH: Function: get_local_unvisited_vertices_gt :: local_nodes: {}".format(local_vertices))
            print("ENV GRAPH: Function: get_local_unvisited_vertices_gt :: del_idx: {}".format(del_idx))

        for idx in sorted(del_idx, reverse=True):
            del (local_vertices[idx])

        # print the unvisited local nodes

        if debug_print:
            print("ENV_GRAPH: GET_LOCAL_UNvisited_vertices :: Unvisited Local Nodes ::: {}".format(local_vertices))
    ...
    def get_remote_nodes(self, node, debug_print=False):
        ...
        if debug_print:
            ...
            print("ENV GRAPHS: get_remote_nodes:: explored nodes: {}".format(self.visited_vertices))
    ...
    def get_all_unvisited_vertices_with_path_cost(self, node, debug_print=True):
        distance_all_nodes, source_all_nodes = self._get_cost_to_all_nodes(node, debug_print)
        ...
        if debug_print:
            print("ENV GRAPHS: get_all_unvisited_vertices_with_path_cost:: nodes to be deleted: {}".format(del_node))
    ...
    def go_local_node(self, node_idx, debug_print=True):
        if debug_print:
            print("ENV GRAPHS:  GO LOCAL NODE: explored nodes before update: {}".format(self.visited_vertices))
    ...
    def _get_cost_to_all_nodes(self, from_node, debug_print=True):
        ...
        # -- mark visited nodes
        visited_vertices = []
        visited_vertices.append(from_node)
        if debug_print:
            print("ENV_GRAPH: _get_cost_to_all_nodes:: Visited Nodes: ", visited_vertices)
            print("ENV_GRAPH: _get_cost_to_all_nodes:: discovered vertices: ", self.discovered_vertices)
        nsteps = 0
        last_node = None
        cur_node = from_node
        cost_to_cur_node = 0
        ...
        while True:
            ...
            if cur_node not in visited_vertices:
                if debug_print:
                    print("\tNode {}, not in Visited Nodes".format(cur_node))
                visited_vertices.append(cur_node)

            # BEGIN debug print
            if debug_print:
                print("\tCurrent Node: ", cur_node)
                print("\tVisited Nodes: ", visited_vertices)
    ...
    # def _get_local_nodes_explored(self, node, gt_explored=None):
    ...
    def set_up_initial_robot_position(self, position, debug_print=True):
        ...
        # -- return the explored graph_explored, visited_vertices, and discovered vertices
    ...
    # --------------------------------------------------------------------------
    # Function: _initialize_ground_truth_graph
    # --------------------------------------------------------------------------
    # Initialize a graph from a given filename
    # --------------------------------------------------------------------------
    def _initialize_ground_truth_graph(self, filename, debug_print=False):
        count = 0
        self.edgeList = []
        self.n_edges = 0
